            ...
            <div class="content ">
                    ...
                    <#include "common/leftbar.ftl">
                        <div class="rside">
                            ...
                            <div class="compang_info clearfix">
                                <dl>
                                    <#if user??>
                                        <dt>${user.companyInfo.companyName}<span id="yyzzsfrz">  已认证</span> <strong>&nbsp</strong></dt>
                                        <dd>信用 ：发货${user.companyInfo.fhds}次 &nbsp&nbsp 装货率${user.companyInfo.zhl} % &nbsp&nbsp结款率${user.companyInfo.zhl} % </dd>
                                        <dd>总运量 ：暂无数据</dd>
                                    </#if>
                                </dl>
                                <dl>
                                    <dt><strong>${user.companyInfo.linkMobile}</strong>&nbsp</dt>
                                    <dd>注册时间 ：${user.companyInfo.operateTime?string("YYYY年MM月dd日")}</dd>
                                    <dd>偏好 ：暂无数据</dd>
                                </dl>
                                <dl>
                                    <dt>&nbsp <strong>&nbsp</strong></dt>
                                    <dd>&nbsp</dd>
                                    <dd>最大装载量：暂无数据</dd>
                                </dl>
                            </div>
               ...
               &nbsp&nbsp&nbsp&nbsp &nbsp总&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp价
                                ...
                                <p class="remark">备&nbsp&nbsp&nbsp&nbsp&nbsp 注
                                 ...
                                 <p class="bttn"><button class="cancel" id="cancelIssue">取消货源</button><button class="sure"  onclick="info_fill()">发布</button></p>
                            </div>
                            <div class="history">
                                <h5>历史发货记录 <a href="#">更多>></a></h5>
                                <ul class="clearfix">
                                    <#if cargoList??>
                                ...
                                <#if cargo.carLength==0>其它车长
                                 ...
                                 <#else></#if>&nbsp &nbsp<#if cargo.carType==0>其它车型
            ...
            <#include "common/foot.ftl">
                ...
                $(document).ready(function() {
                    $(".li_7").addClass("active");
                     ...
                     showDialog() ;
                    if (${user.companyInfo.yyzzsfrz} === 10) {
                        $("#yyzzsfrz").text("已认证");
                    } else {
                        $("#yyzzsfrz").text("未认证");
...
$("#cancelIssue").bind("click",function(){
    var operationCause =$.trim(prompt("请填写操作理由",""));
      if(operationCause) {
      $.ajax({  
              url:"/ajax/untaskcargo.htm",  
              type:"post", 
              data:{userId:${user.id},failReason:operationCause },
              dataType:"json",
              success:function(data) { 
                if (data.flag =1) {
                alert("取消成功");
                  window.location.href = "/taskcargo.htm";
                } else {
                  alert(data.msg); 
                }
              },
